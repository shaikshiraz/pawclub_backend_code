const {adoption,validate}=require('../models/m-adoption');
...
router.get('/',async(req,res)=>{
    const m=await adoption.find().sort('name');
    res.send(m);
...
router.put('/:id',async(req,res)=>{
    ...
    if(error)  return res.status(400).send(error.details[0].message);
        try{
            //look up the dog
            const m=await adoption.findByIdAndUpdate(req.params.id,{name:req.body.name},{
                new:true
            });
   ...
   router.delete('/:id',async(req,res)=>{
    ...
    try{
        //look up the do
        const m=await adoption.findByIdAndRemove(req.params.id);
   ...
   router.get('/:id',async(req,res)=>{
     try{
         ////look up the dog
        const m=await adoption.findById(req.params.id);
        res.send(m);
     }
     //if not exists return 404 error
     catch(ex){
