const {lost,validate}=require('../models/m-lost-pet');
...
router.get('/',async(req,res)=>{
    const lostpet=await lost.find().sort('name');
    res.send(lostpet);
...
router.put('/:id',async(req,res)=>{
    ...
    if(error)  return res.status(400).send(error.details[0].message);
        try{
            //look up the dog
            const lostpet=await lost.findByIdAndUpdate(req.params.id,
                {
                    //basic details
            name:req.body.name,firstname:req.body.firstname, lastname:req.body.lastname,phoneno:req.body.phoneno,
            email:req.body.email,address1:req.body.address1,address2:req.body.address2,city:req.body.city,state:req.body.state,
            ...
            res.send(lostpet);//return updated dog 
           }
   ...
   router.delete('/:id',async(req,res)=>{
    ...
    try{
        //look up the do
        const lostpet=await lost.findByIdAndRemove(req.params.id);
        res.send(lostpet);//return delete do
       }
       //if not exists return 404 error
    catch(ex){
