const {medication,validate}=require('../models/m-medications');
...
router.get('/',upload.single('image'),async(req,res)=>{
    const m=await medication.find().sort('name');
    res.send(m);
...
router.put('/:id',async(req,res)=>{
    ...
    if(error)  return res.status(400).send(error.details[0].message);
        try{
            //look up the dog
            const m=await medication.findByIdAndUpdate(req.params.id,
                {
                    //basic details
   ...
   router.delete('/:id',async(req,res)=>{
    ...
    try{
        //look up the do
        const m=await medication.findByIdAndRemove(req.params.id);
   ...
   router.get('/:id',async(req,res)=>{
     try{
         ////look up the dog
        const m=await medication.findById(req.params.id);
        res.send(m);
     }
     //if not exists return 404 error
     catch(ex){
