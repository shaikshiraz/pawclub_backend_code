const {pet,validate}=require('../models/m-pet-profile');
...
router.get('/',async(req,res)=>{
    const memory=await pet.find().sort('name');
    res.send(memory);
...
router.put('/:id',async(req,res)=>{
    ...
    if(error)  return res.status(400).send(error.details[0].message);
        try{
            //look up the dog
            const memory=await pet.findByIdAndUpdate(req.params.id,
                {
                    //basic details
            name:req.body.name,firstname:req.body.firstname, lastname:req.body.lastname,phoneno:req.body.phoneno,
            email:req.body.email,address1:req.body.address1,address2:req.body.address2,city:req.body.city,state:req.body.state,
            ...
            res.send(memory);//return updated dog 
           }
   ...
   router.delete('/:id',async(req,res)=>{
    ...
    try{
        //look up the do
        const memory=await pet.findByIdAndRemove(req.params.id);
        res.send(memory);//return delete do
       }
       //if not exists return 404 error
    catch(ex){
   ...
   router.get('/:id',async(req,res)=>{
     try{
         ////look up the dog
        const memory=await pet.findById(req.params.id);
        res.send(memory);
     }
     //if not exists return 404 error
     catch(ex){
